...
public class ConnectFourModel extends Observable {
	...
	protected void setRow(String[] savedCoins, int row) {
		for(int column = 0; column < columns-1; column++) {
			if(savedCoins[column] != null) {
				switch (savedCoins[column]) {
				case "1":
					celGrid[row][column] = celStatus.PLAYER1;
					player = celStatus.PLAYER1;
					rounds++;
					break;
				case "2":
					celGrid[row][column] = celStatus.PLAYER2;
					player = celStatus.PLAYER2;
					rounds++;
					break;
				default:
					celGrid[row][column] = celStatus.EMPTY;
					break;
				}
			}
			else {
